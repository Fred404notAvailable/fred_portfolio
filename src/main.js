...
function initProjectCardTilt() {
    ...
    cards.forEach((card, index) => {
        ...
        card.addEventListener('mouseenter', () => {
            ...
            if (map) {
                gsap.fromTo(map, { attr: { scale: 65 } }, { attr: { scale: 0 }, duration: 1.2, ease: 'power2.out' });
            }
        });
        card.addEventListener('mousemove', (e) => {
            ...
            if (setTransZ)
                setTransZ(45); // push text forward in 3D space
        });
        card.addEventListener('mouseleave', () => {
            ...
            if (map) {
                gsap.fromTo(map, { attr: { scale: 45 } }, { attr: { scale: 0 }, duration: 1.0, ease: 'power2.out' });
            }
        });
    });
}
...
function initContactTerminal() {
    ...
    if (!terminal || !nameInput || !emailInput || !msgInput)
        return;
